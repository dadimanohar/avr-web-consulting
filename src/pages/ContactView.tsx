  ...
  return (
    <div className="space-y-12 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-7 bg-navy-900 border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6">
          <div className="border-b border-indigo-950 pb-4">
            <h3 className="text-white font-display font-semibold text-base sm:text-lg flex items-center space-x-2">
              <Sparkles className="text-brand-blue-500 w-4.5 h-4.5 animate-pulse" />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 uppercase">Your Full Name</label>
                <input
                  type="text"
                  required
                  placeholder="e.g., Sandeep Mishra"
                  className="w-full bg-[#05070c] border border-slate-700 rounded-lg px-4 py-2 text-slate-200 outline-none focus:border-brand-blue-500 text-xs sm:text-sm"
                  value={formData.name}
              ...
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 uppercase">Business Email</label>
                <input
                  type="email"
                  required
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 uppercase">Phone Number</label>
                <input
                  type="tel"
                  required
              ...
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 uppercase">Target Domain (Optional)</label>
                <input
                  type="url"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 uppercase">Target Campaign Hub</label>
                <select
                  className="w-full bg-[#05070c] border border-slate-700 rounded-lg px-4 py-2 text-slate-200 outline-none focus:border-brand-blue-500 text-xs sm:text-sm"
              ...
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 uppercase">Primary City Landmark Unit</label>
                <select
                  className="w-full bg-[#05070c] border border-slate-700 rounded-lg px-4 py-2 text-slate-200 outline-none focus:border-brand-blue-500 text-xs sm:text-sm"
                  value={formData.city}
            ...
            <div className="space-y-2">
              <label className="text-xs font-mono text-slate-400 uppercase">Briefly describe your current search ranking obstacles</label>
              <textarea
                rows={4}
                placeholder="Describe page indexation alerts, missing citations, or general keyword drop concerns..."
                className="w-full bg-[#05070c] border border-slate-700 rounded-lg p-4 text-slate-200 outline-none focus:border-brand-blue-500 text-xs sm:text-sm"
            ...
            <button
              ...
              className="w-full bg-brand-blue-500 hover:bg-brand-blue-600 text-white font-medium py-3 rounded-lg flex items-center justify-center space-x-2 transition-all cursor-pointer shadow-lg shadow-brand-blue-500/10 disabled:opacity-50"
            >
          ...
          {success && (
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-start space-x-3 text-emerald-400">
              ...
              <div>
                <p className="text-xs sm:text-sm font-semibold">Consulatation Session Registered Successfully!</p>
                <p className="text-slate-400 text-xs leading-relaxed mt-0.5">We have stored your entry inside the local indexed database. Our search strategist will ping you with audit roadmap coordinates within 24 hours.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-navy-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            <h3 className="text-white font-display font-semibold text-sm flex items-center space-x-2">
              <MapPin className="text-brand-blue-500 w-4 h-4 shrink-0" />
              <span>Andhra Pradesh Certified Headquarters</span>
            </h3>

            <div className="space-y-3.5 text-xs text-slate-300">
              <div className="flex items-start space-x-3">
                <Map className="w-4 h-4 text-brand-blue-500 shrink-0 mt-0.5" />
              ...
              <div className="flex items-center space-x-3">
                ...
                <p><strong>Hotline:</strong> {COMPANY_CONTACT.phone}</p>
              </div>

              <div className="flex items-center space-x-3">
                <Mail className="w-4 h-4 text-brand-blue-500 shrink-0" />
                <p><strong>Support Email:</strong> {COMPANY_CONTACT.email}</p>
              </div>

              <div className="flex items-center space-x-3">
                <Clock className="w-4 h-4 text-brand-blue-500 shrink-0" />
                <p><strong>Operational Hours:</strong> 9:00 AM - 6:30 PM (IST) | Mon - Sat</p>
              </div>
            </div>
          ...
          {localSubmissionList.length > 0 && (
            <div className="bg-navy-900 border border-slate-800 rounded-2xl p-5 space-y-3">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {localSubmissionList.map((lead, idx) => (
                  <div key={idx} className="bg-[#05070c] border border-slate-850 p-2 text-[10px] rounded space-y-1">
                    <div className="flex justify-between text-slate-300">
                      <strong>{lead.name}</strong>
                      <span className="text-brand-blue-500">{lead.city}</span>
                    </div>
                    <p className="text-slate-500">Service: {lead.service} - {lead.email}</p>
                    <p className="text-slate-400 leading-tight italic line-clamp-1">&quot;{lead.message}&quot;</p>
                  </div>
                ))}
              </div>
            </div>
